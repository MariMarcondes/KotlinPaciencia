...
object GameModel {
    ...
    fun onDeckTap() {
        if (deck.cardsInDeck.size > 0) { val card = deck.drawCard(); card.faceUp = true
            wastePile.add(card)
        } else {
            deck.cardsInDeck = wastePile.toMutableList(); wastePile.clear()
        }
    }

    fun onWasteTap() {
        if (wastePile.size > 0) { val card = wastePile.last()
    ...
    fun onFoundationTap(foundationIndex: Int) {
        ...
        if (foundationPile.cards.size > 0) { val card = foundationPile.cards.last()
